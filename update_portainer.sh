...
sudo docker run -d --dns 172.20.0.250 --network=frontend --name=portainer --restart=always -v /var/run/docker.sock:/var/run/docker.sock -v /docker/container/portainer:/data portainer/portainer-ce:latest || error "Failed to execute newer version of Portainer!"
